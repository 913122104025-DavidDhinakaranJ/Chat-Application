        ...
        <div className={`bg-[#818582]/10 h-full p-5 rounded-r-xl overflow-y-scroll text-white ${selectedUser ? 'max-md:hidden' : ''} `}>
            <div className="pb-5">
                ...
                <div className="relative py-2 group">
                    <img src={assets.more} alt="avatar" className="max-h-5 cursor-pointer" />
                    <div className="absolute top-full right-0 z-20 w-32 p-5 rounded-md bg-[#282142] border border-gray-600 shadow-lg text-gray-100 hidden group-hover:block">   
        ...
        <div className="flex flex-col gap-3">  
            {filteredUsers.map((user, index) => (
                <div onClick={() => {setSelectedUser(user), setUnseenMessages(prev => ({...prev, [user._id]:0}))}} key={index} className={`relative flex items-center gap-2 p-2 pl-4 rounded cursor-pointer max-sm:text-sm ${selectedUser?._id === user._id && 'bg-[#282142]/50'}`}>
                    <img src={user?.profilePicture || assets.avatar} alt="" className="w-[35px] aspect-[1/1] rounded-full"/>
                    <div className="flex flex-col leading-5">
                        <p>{user.username || "User"}</p>
                        {
                            onlineUsers.includes(user._id) ? <span className="text-xs text-green-400">Online</span> : <span className="text-xs text-red-400">Offline</span>
                        }
                    </div>
